...
function reducer(state = reducerInitialState, action) {
	switch (action.type) {
		...
		case "someoneChat":
			return { ...state, chatings: insertItem(state.chatings, action) };
		case "someoneExit":
		...
		case "roomExit":
			return { ...state, users: [], chatings: [], myRoom: "init" };
		case "start":
			return {
				...state,
				state: "게임중",
		...
		case "blocks":
			...
			if (enemyNum === 0) {
				return { ...state, blocks2: action.data.blocks };
			} else if (enemyNum === 1) {
				return { ...state, blocks3: action.data.blocks };
			} else if (enemyNum === 2) {
				return { ...state, blocks4: action.data.blocks };
			} else if (enemyNum === 3) {
				return { ...state, blocks5: action.data.blocks };
			} else if (enemyNum === 4) {
				return { ...state, blocks6: action.data.blocks };
			} else {
				return state;
			}
		case "gameset":
		...
		case "gamesetTome":
			return { ...state, state: "대기중", personNum: --state.personNum };
		case "lineup":
			if (state.gameset === 0) {
				return {
					...state,
