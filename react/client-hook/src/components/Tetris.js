...
class Tetris extends Component {
	...
	_chatFun = () => {
		this.props.dispatch({
			type: "server/speak",
			data: {
				speaker: this.props.me,
				content: this.state.text,
				myRoom: this.props.myRoom,
			},
		}); //client -> server
		this._input.value = "";
		this.setState({
	...
	_exitFun = () => {
		this.props.dispatch({
			type: "server/exit",
			data: {
				roomName: this.props.myRoom,
				user: this.props.me,
				someoneNum: this.props.myNum,
			},
		}); //client -> server
	};
	...
	_startFun = () => {
		...
		this.props.dispatch({
			type: "server/start",
			data: { roomName: this.props.myRoom },
		}); //client -> server
	};
	...
	_makeNewPreviewFun = () => {
		...
		for (let i = 0; i < length - 4; i++) {
			for (let j = length - 4; j < length; j++) {
				if (
					this.state.blocks[i][2] === this.state.blocks[j][2] &&
					this.state.blocks[i][3] === this.state.blocks[j][3]
				) {
	...
	_lineClear = () => {
		...
		for (let i = length - 4; i < length; i++) {
			...
			if (myTopNumber.length > 9) {
				...
				this.props.dispatch({
					type: "server/lineup",
					data: { roomName: this.props.myRoom, randomVar: randomVar },
				}); //client -> server
			}
		}
		this.setState({
	...
	_lineupFun = () => {
		...
		if (this.gage > this.props.personNum - 2) {
	...
	render() {
		return (
			<div className="body">
				...
				{this.props.state !== "게임중" &&
					this.props.users[0] === this.props.me && (
						<button
							className="start"
							key="start"
							onClick={() => this._startFun()}
						>
							시작하기
						</button>
					)}
				{this.props.state === "대기중" ? (
					<Link to="/" onClick={() => this._exitFun()}>
